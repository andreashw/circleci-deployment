...
export default function Users(/*props*/) {
  ...
  const onSearch = (search:any) => {
    ...
    const filteredUsers = users.filter(
      (user: { name: string; email: string; address: string; workplace: string; phone: string | any[]; }) =>
  ...
  const onSubmitEditForm = (oldUser:any, newUser:any) => {
    toggleDrawer(false);
    ...
    tmpUsers = tmpUsers.filter((u: any) => u !== oldUser);
    setUsers(tmpUsers);
  ...
  const sendMessage = (user: any) => {
    const onSubmit = (subject: any, message: any) => {
  ...
  const onSendMessage = (user: { email: any; }, subject: any, message: any) => {
  ...
  const copyProfile = (user: { name: any; }) => {
  ...
  const deleteProfile = (user: { name: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | ReactFragment | ReactPortal | null | undefined; }) => {
  ...
  const onDeleteProfile = (user: { name: any; }) => {
    // remove data in db

    let tmpUsers = users;
    tmpUsers = tmpUsers.filter((u: any) => u !== user);
    setUsers(tmpUsers);
